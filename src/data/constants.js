export const Bio = {
  name: "Mohamed Dhia Ghorbel",
  roles: ["Full Stack Developer", "UI Designer", "Mobile Developer"],
  description:
    "I'm a Bachelor's Graduate in Information Technology focusing on Information Systems Development, eager to leverage my academic knowledge and practical experience to contribute effectively in a dynamic professional environment.",
  github: "https://github.com/Shaclight69",
  linkedin: "https://www.linkedin.com/in/shaclight/",
  insta: "https://www.instagram.com/cxldghoste/",
  facebook: "https://www.facebook.com/dhiaghorbel69/",
  resume: "resume.pdf",
};

export const skills = [
  {
    title: "Frontend",
    skills: [
      {
        name: "React",
        image: "https://skillicons.dev/icons?i=react",
      },
      {
        name: "Redux",
        image: "https://skillicons.dev/icons?i=redux",
      },
      {
        name: "Material-UI",
        image: "https://skillicons.dev/icons?i=materialui",
      },
      {
        name: "Next.js",
        image: "https://skillicons.dev/icons?i=nextjs",
      },
      {
        name: "Tailwind",
        image: "https://skillicons.dev/icons?i=tailwind",
      },
      {
        name: "Styled-components",
        image: "https://skillicons.dev/icons?i=styledcomponents",
      },
      {
        name: "HTML",
        image: "https://skillicons.dev/icons?i=html",
      },
      {
        name: "CSS",
        image: "https://skillicons.dev/icons?i=css",
      },
      {
        name: "Bootstrap",
        image: "https://skillicons.dev/icons?i=bootstrap",
      },
    ],
  },
  {
    title: "Backend",
    skills: [
      {
        name: "Node.js",
        image: "https://skillicons.dev/icons?i=nodejs",
      },
      {
        name: "Express.js",
        image: "https://skillicons.dev/icons?i=express",
      },
      {
        name: "MongoDB",
        image: "https://skillicons.dev/icons?i=mongodb",
      },
      {
        name: "Prisma",
        image: "https://skillicons.dev/icons?i=prisma",
      },
      {
        name: "Supabase",
        image: "https://skillicons.dev/icons?i=supabase",
      },
      {
        name: "Firebase",
        image: "https://skillicons.dev/icons?i=firebase",
      },
      {
        name: "Stripe",
        image:
          "https://assets.ctfassets.net/fzn2n1nzq965/01hMKr6nEEGVfOuhsaMIXQ/c424849423b5f036a8892afa09ac38c7/favicon.ico",
      },
      {
        name: "Cloudinary",
        image: "https://www.svgrepo.com/show/353566/cloudinary.svg",
      },
      {
        name: "JWT",
        image: "https://cdn.worldvectorlogo.com/logos/jwt-3.svg",
      },
    ],
  },
  {
    title: "Languages",
    skills: [
      {
        name: "C++",
        image: "https://skillicons.dev/icons?i=cpp",
      },
      {
        name: "Javascript",
        image: "https://skillicons.dev/icons?i=js",
      },
      {
        name: "Java",
        image: "https://skillicons.dev/icons?i=java",
      },
      {
        name: "Python",
        image: "https://skillicons.dev/icons?i=py",
      },
      {
        name: "C#",
        image: "https://skillicons.dev/icons?i=cs",
      },
      {
        name: "Typescript",
        image: "https://skillicons.dev/icons?i=ts",
      },
    ],
  },
  {
    title: "Tools",
    skills: [
      {
        name: "Git",
        image: "https://skillicons.dev/icons?i=git",
      },
      {
        name: "GitHub",
        image: "https://skillicons.dev/icons?i=github",
      },
      {
        name: "Vercel",
        image: "https://skillicons.dev/icons?i=vercel",
      },
      {
        name: "Netlify",
        image: "https://skillicons.dev/icons?i=netlify",
      },
      {
        name: "VSCode",
        image: "https://skillicons.dev/icons?i=vscode",
      },

      {
        name: "Postman",
        image: "https://skillicons.dev/icons?i=postman",
      },
      {
        name: "Lucidchart",
        image:
          "https://cdn-cashy-static-assets.lucidchart.com/marketing/images/Lucidchart_favicon_full_96x96.png",
      },
      {
        name: "Canva",
        image: "https://static.canva.com/static/images/favicon-1.ico",
      },
    ],
  },
];

export const experiences = [
  {
    id: 0,
    img: "https://www.ftbb.org.tn/wp-content/uploads/2017/09/logo-laposte.png",
    role: "Full Stack Developer Intern",
    company: "The Tunisian Post",
    date: "February 2023 - June 2023",
    desc: "Concepted, designed, and developed a stamps e-commerce application with chatbot integration streamlining the management of philatelic activities within Tunisia.",
    skills: [
      "React",
      "Redux",
      "Node.js",
      "Express.js",
      "MongoDB",
      "Cloudinary",
      "Stripe",
      "JWT",
      "Material-UI",
      "Styled-components",
      "CSS",
      "JavaScript",
      "Dialogflow",
      "Kommunicate",
      "Postman",
      "Lucidchart",
      "Git",
    ],
  },
  {
    id: 1,
    img: "https://www.ftbb.org.tn/wp-content/uploads/2017/09/logo-laposte.png",
    role: "Software Developer Intern",
    company: "The Tunisian Post",
    date: "January 2023",
    desc: "Conceived and implemented an admin dashboard with empowering administrators with data grids and graphical representations, offering insights into the company's performance.",
    skills: [
      "React",
      "Redux",
      "Node.js",
      "Express.js",
      "MongoDB",
      "Material-UI",
      "Nivo",
      "Javascript",
      "Postman",
      "Lucidchart",
      "Git",
      "Render",
    ],
  },
  {
    id: 2,
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSBXE9c54PDm7X_abie6K9or2AZ8T0frVwvZ60ZLjQOkcZD9QrxgGzWzMblr_GJEmpjO3k&usqp=CAU",
    role: "Junior Developer Intern",
    company: "TIS Circuits",
    date: "February 2022 ",
    desc: "Built an employment recruitment web application with C# to enhance operational efficiency for human resources professionals overseeing the hiring process.",
    skills: [
      "ASP.NET MVC",
      "C#",
      "JQuery",
      "Bootstrap",
      "SQL Server Management Studio",
      "Microsoft SQL Server",
    ],
  },
  {
    id: 3,
    img: "data:image/jpeg;base64,",
    role: "Student Intern",
    company: "Afrochim",
    date: "June 2021",
    desc: "Presented a brief overview of the introductory professional life internship to gain initial practical experience in a professional work environment.",
  },
];

export const projects = [
  {
    id: 0,
    title: "Socialize Chatapp",
    date: "Sep 2023 - Oct 2023",
    description:
      "Directed development of a Chat App utilizing Typescript, and admitting Pusher for real-time communication. Utilized Prisma with MongoDB for streamlined database operations, integrated user authentication featuring Google and GitHub integration using NextAuth, and incorporated file-sharing capabilities with Cloudinary. Designed and implemented sleek responsive UI using React and NextJS, TailwindCSS. Applied Git for version control and executed the application deployment on the Vercel platform.",
    image: "https://imgur.com/uJIHB3h.png",
    tags: [
      "Next.js",
      "Typescript",
      "React",
      "TailwindCSS",
      "MongoDB",
      "Prisma",
      "Pusher",
      "Cloudinary",
      "Vercel",
      "Git",
    ],
    category: "personal app",
    github: "https://github.com/Shaclight69/socialize_chatapp",
    webapp: "https://socialize-chatapp.vercel.app/",
  },
  {
    id: 1,
    title: "Spotify Clone",
    date: "Aug 2023 - Sep 2023",
    description:
      "Initiated the creation of a Spotify Clone employing Stripe for premium subscription payments. Conducted Supabase for database management, storage, and authentication featuring GitHub integration. Crafted a fully responsive UI using React, NextJS, and TailwindCSS, and employed Git for version control. Concluded the development lifecycle with the deployment of the application on Vercel.",
    image: "https://imgur.com/dZ5YO6B.png",
    tags: [
      "Next.js",
      "Typescript",
      "React",
      "TailwindCSS",
      "Supabase",
      "PostgreSQL",
      "Stripe",
      "Vercel",
      "Git",
    ],
    category: "personal app",
    github: "https://github.com/Shaclight69/spotify-clone",
    webapp: "https://xspotify-clone.vercel.app/",
  },
  {
    id: 2,
    title: "Scraprice",
    date: "Jul 2023",
    description:
      "Developed the Scraprice project a solution tracking product prices on Amazon by scraping their details, utilizing Next.js and TailwindCSS for Frontend. Implemented data scraping functionalities using Cheerio, storing the extracted details in a MongoDB database. Integrated Nodemailer to automate email notifications, ensuring subscribed users are promptly informed of any changes to their monitored products. Applied Git for version control and executed the application deployment on the Vercel platform.",
    image: "https://imgur.com/cSK12Cl.png",
    tags: [
      "Next.js",
      "Javascript",
      "React",
      "TailwindCSS",
      "MongoDB",
      "Cheerio",
      "Nodemailer",
      "Vercel",
      "Git",
    ],
    category: "personal app",
    github: "https://github.com/Shaclight69/scraprice_cron",
    webapp: "https://scraprice.vercel.app/",
  },
  {
    id: 3,
    title: "eStampsONP",
    date: "Feb 2023 - Jun 2023",
    description:
      "Oversaw the development of a Full Stack MERN stamps e-commerce with ChatBot integration, implementing user authentication with MongoDB including JSONWebToken middleware, enforcing Cloudinary for image upload, and merging Stripe for subscription management and payment transactions. Assembled a responsive UI using ReactJS, MaterialUI, and Styled-components, applied Redux for state management, developed an admin dashboard to control user activities, product management, and order processing, and conducted Git for version control.",

    image: "https://imgur.com/PbNQWvn.png",
    tags: [
      "React",
      "Redux",
      "Node.js",
      "Express.js",
      "MongoDB",
      "Cloudinary",
      "Stripe",
      "JWT",
      "Material-UI",
      "Styled-components",
      "CSS",
    ],
    category: "internship app",
    github: "https://github.com/Shaclight69/e-stampsonp",
  },
  {
    id: 4,
    title: "Post Admin Dashboard",
    date: "Jan 2023",
    description:
      "Led the creation of a Post Office admin dashboard using the MERN Stack featuring Redux Toolkit Query. Designed and implemented responsive, user-friendly UI using ReactJS, MaterialUI, and Nivo. Applied Git for version control, and executed the application deployment on Render.",
    image: "https://imgur.com/OjFAIvq.png",
    tags: [
      "React",
      "Redux",
      "Node.js",
      "Express.js",
      "MongoDB",
      "Material-UI",
      "Nivo",
      "Javascript",
      "Postman",
      "Git",
      "Render",
    ],
    category: "internship app",
    github: "https://github.com/Shaclight69/post-office_dashboard",
    webapp: "https://postofficedashboardui.onrender.com/",
  },
];
